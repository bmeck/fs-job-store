...
//
const debug = !1 ? Function.prototype : console.log;
...
class JobStore {
  ...
  pullJob(fn) {
    ...
    task(function* (next, abort) {
      const cleanup = new FinallyAggregator();
      try {
        const potential_job_ids = yield _ => fs.readdir(_penddir_raw(self.__dirname), _);
        let id;
        let release;
        let ret;
  ...
  pullNotification(fn) {
    ...
    task(function* (next, abort) {
      const cleanup = new FinallyAggregator();
      try {
        const potential_notification_ids = yield _ => fs.readdir(_waitdir_raw(self.__dirname), _);
        let id;
        let release;
        let ret;
